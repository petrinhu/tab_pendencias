...
| README-1 | W10 | Docs | README bilíngue corrigido para **9 colunas** (documentava 8 em `README.md:34-37` e no espelho en `:131-134`), com matriz de instalação, as duas camadas (núcleo genérico × convenções da casa), as dependências externas hoje não declaradas, e os modos degradados (sem agents → thread direta; sem vault → wikilinks mortos; sem python3 → só a parte agent-driven; Windows sem Git for Windows → perde só o sync mecânico via hook). **DoD inclui o check de consistência README × SKILL.md** -- o gate que teria pegado o schema obsoleto; conferido manualmente nesta fatia (schema de 9 colunas idêntico nos dois arquivos). Falta ainda documentar o comportamento fino de `--audit`/`--fix` (checks, formato de relatório, opções de CLI), que segue em implementação por outras fatias -- por isso o status permanece "em andamento", não "pendente verificação". | Alta | SKILL-AUDIT, FIX-ENG | Média | 🔄 Em andamento | — |
